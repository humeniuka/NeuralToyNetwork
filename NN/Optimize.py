...
from eFF import Constraints   # contains definition of log-barrier
from eFF import Hessian       

    
def line_search_backtracking(xk, fk, grad_fk, pk, func,
                             constraints=None,
                             a0=1.0, rho=0.3, c=0.0001, lmax=5):
    """
...
def minimize(objfunc, x0,
    ...
    line_search_method: choose how the step length in the search direction should be determined
       ...
       'Wolfe'            - attempts to find a step length that satisfies Wolfe's conditions
        None              - No line search is performed, x(i+1) = x(i) - a*df/dx,
                            the step length is taken from the option `steplen`
    constraints: callable, constraints(x) should return a vector C with the values 
              of the constraints at x and matrix A whose rows are the gradients 
              of the constraints w/r/t to x
    steplen: default step length if no line search should be performed 
             (only used if line_search_method=None)
    max_steplen: callable, `max_steplen(x,v)` computes the largest step length `amax` such that
              x + amax*v  lies inside the feasible set or on its boundary.
    ...
    assert line_search_method in ["Armijo", "Wolfe", None]
    n = len(x0)
    def barrier(x):
    ...
    def hess(x):
        H = Hessian.numerical_hessian_G(grad, x)
        #print "condition number k=%e" % Hessian.condition_number(H)
        return H
    ...
    for k in range(0, maxiter):
        ...
        elif line_search_method == None:
            # simply take a step along the negative gradient
            x_kp1 = xk + steplen * pk
        ...
        #raise RuntimeError("No convergence with optimizer '%s' after %d iterations!" % (method, k+1))
        print "WARNING: No convergence with optimizer '%s' after %d iterations!" % (method, k+1)
...
def check_gradient(f, x0):
    ...
    print "grad (NUMERICAL)  = %s" % Hessian.numerical_gradient(func, x0)
